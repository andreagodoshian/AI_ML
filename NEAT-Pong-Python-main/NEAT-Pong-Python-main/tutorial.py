...
class PongGame:
    ...
    def train_ai(self, genome1, genome2, config):
        ...
        while run:
            ...
            # (as soon as it misses, it moves on)

            # it's not a perfect method,
            # but it's quick because it's just a game

            # (make sure to add "hits" parameter so it's not infinite)
...
# (because in theory, can't train against a person)
...
def run_neat(config):
    ...
    # every one generation

    winner = p.run(eval_genomes, 50)
    # (reach 400 *or* 50 generations)
